...
export async function transformRequestBody(
	...
	const strippedModel = originalModel?.includes("/")
		? originalModel.split("/").pop()
		: originalModel;
	const normalizedModel = normalizeModel(originalModel);

	const modelLookupCandidates = Array.from(
		new Set(
			[originalModel, strippedModel, normalizedModel].filter(
	...
	const lookupModel =
		modelLookupCandidates.find((candidate) => userConfig.models?.[candidate])
	...
	logDebug(
		...
		{
			lookupCandidates: modelLookupCandidates,
