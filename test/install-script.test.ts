...
describe('Install script', () => {
	it('updates existing JSONC and preserves comments', () => {
		...
		const configPath = writeConfig(
			...
			`{
  // My existing config
  "plugin": ["some-other-plugin@1.2.3", "opencode-codex-auth@4.2.0"],
  "provider": {
    "openai": {
      "timeout": 60000,
      "models": { "custom-model": { "name": "Custom" } }
		...
		expect(data.plugin).toContain('opencode-codex-auth');
		...
		expect(data.provider.openai.models['gpt-5.3-codex']).toBeDefined();
	...
	it('prefers JSONC when both jsonc and json exist', () => {
		...
		const jsoncPath = writeConfig(
			homeDir,
			'opencode.jsonc',
			`{ "plugin": ["opencode-codex-auth@4.2.0"] }`,
		);
  ...
  "plugin": ["some-other-plugin@1.2.3", "opencode-codex-auth@4.2.0"],
	...
	it('uninstall --all removes plugin artifacts', () => {
		...
		writeFileSync(join(opencodeDir, 'codex-auth-config.json'), '{}');
		writeFileSync(join(opencodeDir, 'codex-auth-accounts.json'), '{}');
		...
		writeFileSync(join(opencodeDir, 'cache', 'gpt-5.3-codex-instructions.md'), 'cache53');
		writeFileSync(join(opencodeDir, 'cache', 'gpt-5.3-codex-instructions-meta.json'), '{}');

		runInstaller(['--uninstall', '--all', '--no-cache-clear'], homeDir);

		expect(existsSync(join(opencodeDir, 'auth', 'openai.json'))).toBe(false);
		expect(existsSync(join(opencodeDir, 'codex-auth-config.json'))).toBe(false);
		expect(existsSync(join(opencodeDir, 'codex-auth-accounts.json'))).toBe(false);
		...
		expect(existsSync(join(opencodeDir, 'cache', 'gpt-5.3-codex-instructions.md'))).toBe(false);
		expect(existsSync(join(opencodeDir, 'cache', 'gpt-5.3-codex-instructions-meta.json'))).toBe(false);
	});
});
