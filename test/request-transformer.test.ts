...
describe('Request Transformer Module', () => {
	...
	describe('addCodexBridgeMessage', () => {
		...
		it('should default gpt-5.3-codex to high effort', async () => {
			const body: RequestBody = {
				model: 'gpt-5.3-codex',
				input: [],
			};
		...
		it('should apply per-model options when model includes provider prefix', async () => {
			const body: RequestBody = {
				model: 'openai/gpt-5.3-codex-xhigh',
				input: [],
			};
			const userConfig: UserConfig = {
				global: { reasoningEffort: 'medium', reasoningSummary: 'auto', textVerbosity: 'medium' },
				models: {
					'gpt-5.3-codex-xhigh': {
						options: { reasoningEffort: 'xhigh', reasoningSummary: 'detailed', textVerbosity: 'high' },
					},
				},
			};
			const result = await transformRequestBody(body, codexInstructions, userConfig);
