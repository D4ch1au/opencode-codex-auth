...
const PLUGIN_NAME = "opencode-codex-auth";
...
const resolvedHomeDir = process.env.HOME || process.env.USERPROFILE || homedir();
...
const configDir = join(resolvedHomeDir, ".config", "opencode");
const configPathJson = join(configDir, "opencode.json");
const configPathJsonc = join(configDir, "opencode.jsonc");
const cacheDir = join(resolvedHomeDir, ".cache", "opencode");
...
const opencodeAuthPath = join(resolvedHomeDir, ".opencode", "auth", "openai.json");
const pluginConfigPath = join(
	resolvedHomeDir,
	".opencode",
	"codex-auth-config.json",
);
const accountPoolPath = join(
	resolvedHomeDir,
	".opencode",
	"codex-auth-accounts.json",
);
const pluginLogDir = join(resolvedHomeDir, ".opencode", "logs", "codex-plugin");
const opencodeCacheDir = join(resolvedHomeDir, ".opencode", "cache");
...
async function clearPluginArtifacts() {
	...
	const cacheFiles = [
		...
		"gpt-5.3-codex-instructions-meta.json",
		"opencode-codex.txt",
